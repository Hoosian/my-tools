...
import fs from 'fs';
import { convertFile, convertDirectory, getSupportedFormats, generateOutputFilename, parseRatio, parseSize } from './converter.js';
...
program
  ...
  .action(async (inputPath, options) => {
    const { format, quality, recursive, ratio, size } = options;
    ...
    const opts = { quality: qualityNum, skipExisting: true, recursive, ratio: ratioObj, size: sizeObj };
...
async function interactiveMenu() {
  ...
  if (action === 'single') {
    const { inputFile, format, quality, ratio, size } = await inquirer.prompt([
      { type: 'input', name: 'inputFile', message: 'Input file path:' },
    ...
    const result = await convertFile(inputFile, format, { quality: qualityNum, skipExisting: true, ratio: ratioObj, size: sizeObj });
  ...
  if (action === 'dir') {
    const { dirPath, format, quality, recursive, ratio, size } = await inquirer.prompt([
